...
const GEM_COLORS: Record<string, TileColors> = {
  ...
  '💥': { background: '#2d1400', border: '2px solid #f97316', boxShadow: '0 0 18px rgba(249,115,22,0.9)' },
};

// ── Gem tile component ────────────────────────────────────────────────────────

function GemTile({ symbol, size = 32, highlight = false }: { symbol: string; size?: number; highlight?: boolean }) {
  const colors = GEM_COLORS[symbol] ?? { background: '#0a0a1e', border: '2px solid #6366f1', boxShadow: '0 0 10px rgba(99,102,241,0.5)' };
  ...
  return (
    <div
      style={{
        ...
        lineHeight: 1,
        ...colors,
        ...(highlight ? { border: '2px solid #e0f2fe', boxShadow: '0 0 18px rgba(224,242,254,0.75)' } : {}),
      }}
    >
      {symbol}
    </div>
...
export function renderGemBlastDemo(type: TutorialDemoType, _modeColor: string): React.ReactNode | null {
  const tileSize = 28;
  const gap = 3;
  ...
  switch (type) {
      ...
      return (
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(3, ${tileSize}px)`, gap, marginBottom: 4 }}>
    ...
    case 'gemblast-ready': {
      ...
      return (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 120 }}>
          {['💎', '💍', '🔮', '🟣', '🔵', '💥'].map((sym) => (
