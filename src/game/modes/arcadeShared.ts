...
function fillColumn(
  ...
  for (let i = 0; i < emptyCount; i++) {
    ...
    result.push(createNewArcadeTile(col, y, activeSymbols, features));
  }
...
export function applyGravity(
  ...
  for (let col = 0; col < gridSize; col++) {
    const columnTiles = tiles.filter((t) => t.x === col).sort((a, b) => b.y - a.y);
    const filledCol = fillColumn(col, columnTiles, gridSize, activeSymbols, features);
    newTiles.push(...filledCol);
  }
