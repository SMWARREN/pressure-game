...
export default function WallOverlay({ wallOffset, gridSize, wallsJustAdvanced, isPlaying }: WallOverlayProps) {
  if (!isPlaying || wallOffset <= 0) return null

  const percentage = (wallOffset / gridSize) * 100
  const transform = wallsJustAdvanced ? '2px' : '0'
  const transition = 'all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)'
  const boxShadow = wallsJustAdvanced ? '0 4px 20px rgba(239,68,68,0.4)' : 'none'
